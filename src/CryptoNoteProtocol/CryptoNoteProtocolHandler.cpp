...
bool CryptoNoteProtocolHandler::request_missing_objects(CryptoNoteConnectionContext& context, bool check_having_blocks) {
  if (context.m_needed_objects.size()) {
    ...
    context.m_state = CryptoNoteConnectionContext::state_normal;
    logger(Logging::INFO, Logging::BRIGHT_GREEN) << context << "SUCCESSFULLY SYNCHRONIZED WITH THE BITCOINNOVA NETWORK.";
    on_connection_synchronized();
  }
  return true;
}
...
bool CryptoNoteProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    logger(Logging::INFO) << ENDL
      << ENDL
      << "Always exit Bitcoinnovad and Simplewallet with the \"exit\" command." << ENDL
      << "If you do not exit properly, you may lose your blockchain and wallet data." << ENDL
      << ENDL
      << "Use \"help\" command to see the list of available commands." << ENDL
      << "Use \"export_keys\" command to display your keys for restoring a corrupted wallet.";
